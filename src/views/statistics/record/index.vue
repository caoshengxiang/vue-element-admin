  ...
  export default {
    ...
    created() {
      if (this.$route.query.type) {
        this.searchForm.type = parseInt(this.$route.query.type, 10)
      }
    },
    mounted() {
      this.chart = this.$echarts.init(document.getElementById('Chart'))
      this.getList()
      this.getBikeRegistrationTotal()
    ...
    methods: {
      ...
      getList() {
        if (!this.searchForm.type) {
          this.searchForm.type = this.bikeCompany[0].value
        }
        this.$api.statistics.licenseChange(this.searchForm).then(da => {
          if (da.data.outData) {
            const names = []
      ...
      /**/
      getBikeRegistrationTotal() {
        this.$api.statistics.bikeRegistrationTotal().then(res => {
          this.bikeRegistrationTotal = res.data
        })
      }
    }
  }
